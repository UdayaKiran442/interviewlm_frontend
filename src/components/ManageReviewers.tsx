...
export default function ManageReviewers({ reviewers }: { reviewers: IReviewer[] }) {
    ...
    function updateReviewerDetails(field: keyof IAddReviewer, value: any) {
        setAddReviewer(previousReviewerDetails => {
            const updateReviewerDetails = { ...previousReviewerDetails, [field]: value }
                    ...
                    <Card className="rounded-xl">
                        ...
                        <div>
                            <div className="pt-4">
                                ...
                                <Input id="name" type="text" placeholder="e.g. John Doe" name="name" value={addReviewer.name} onChange={(e) => updateReviewerDetails("name", e.target.value)} />
                            </div>
                            <div className="mt-2.5">
                                <Label id="email" label="Email" className="mb-1.5" />
                                <Input id="email" type="email" placeholder="e.g. john.doe@example.com" name="email" value={addReviewer.email} onChange={(e) => updateReviewerDetails("email", e.target.value)} />
                            </div>
                            <div className="mt-2.5">
                                <Label id="phone" label="Phone" className="mb-1.5" />
                                <Input id="phone" type="tel" placeholder="e.g. 123-456-7890" name="phone" value={addReviewer.phone} onChange={(e) => updateReviewerDetails("phone", e.target.value)} />
                            </div>
                            <div className="mt-2.5">
                                <Label id="jobTitle" label="Job Title" className="mb-1.5" />
                                <Input id="jobTitle" type="text" placeholder="e.g. Senior Software Engineer" name="jobTitle" value={addReviewer.jobTitle} onChange={(e) => updateReviewerDetails("jobTitle", e.target.value)} />
                            </div>
                            <div className="mt-2.5">
                                <ButtonSecondary disabled={loading} onClick={inviteReviewer} className="w-full flex items-center gap-2 justify-center">
                                    <UserPlus size={18} />
                                    Add Reviewer
                                </ButtonSecondary>
                            </div>
                    ...
                    <Card className="rounded-xl">
                        ...
                        {reviewers.map((reviewer) => (
                            <div key={reviewer.reviewerId}>
                                ...
                                {reviewer.jobs[0].jobId && reviewer.jobs.length > 0 ? <div className="mt-5">
                                    ...
                                    <Tagline className="text-sm">Not Assigned to any job</Tagline>
                                </div> }
